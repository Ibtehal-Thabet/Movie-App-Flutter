...
import 'package:movieflutter/RepeatedFunction/sliderlist.dart';
...
//reminder
  ...
  Future Moviedetails() async {
    ...
    if (moviedetailresponse.statusCode == 200) {
      ...
      String tv_url =
          "https://api.tvmaze.com/singlesearch/shows?q=${MovieDetails.first['title']}";
      var tv_response = await http.get(Uri.parse(tv_url));
      if (tv_response.statusCode == 200) {
        var tvjson = jsonDecode(tv_response.body);
        if (tvjson['schedule']['time'] != null &&
            tvjson['schedule']['time'] != '') {
          schedule.add(tvjson['schedule']['time']);
          schedule.add(tvjson['schedule']['days']);
          String time1 = schedule[0];
          parts = time1.split(":");
          // print('dayyyy$days hhhhhhhhhhhh$hour minnnnnnnnnn$min');
          if (tvjson['network']['name'] == "null") {
            schedule.add(tvjson['webChannel']['name']);
          } else
            schedule.add(tvjson['network']['name']);
        }
      }
    } else {}
              ...
              return CustomScrollView(
                  ...
                  slivers: [
                    ...
                    SliverList(
                      ...
                      //   _scheduleWeeklyNotification(schedule[1][0].toString() as Day,Time(parts[0],parts[1]));
                      // }, icon: Icon(Icons.save), label: Text("reminder")),
                                  ...
                                  onPressed: schedule.isEmpty
                                      ...
                                      : () {
                                          String dayOfWeekString =
                                              schedule[1][0].toString();
                                          Day day = convertStringToDay(
                                              dayOfWeekString.toLowerCase());
                                          _scheduleWeeklyNotification(
                                              day,
                                              Time(int.parse(parts[0]),
                                                  int.parse(parts[1])));
                                          Fluttertoast.showToast(
                                              msg: "Added to reminder",
                      ...
                      Padding(
                          padding: const EdgeInsets.only(left: 20, top: 10),
                          child: tittletext('schedule :')),
                      Padding(
                          padding: const EdgeInsets.only(left: 20, top: 10),
                          child: schedule.isEmpty
                              ? overviewtext("This Movie Will Not Display Soon")
                              : Row(
                                  children: [
                                    Icon(
                                    ...
                                    overviewtext(schedule[1][0].toString()),
                                    Spacer(),
                                    Icon(Icons.timer, color: Colors.white),
                                    overviewtext(schedule.first.toString()),
                                    Spacer(),
                                    Icon(Icons.tv, color: Colors.white),
                                    overviewtext(schedule[2].toString())
                                  ],
                                )),
                      //
  ...
  void _scheduleWeeklyNotification(Day day, Time time) async {
    ...
    // Schedule the notification for the given day and time
    await flutterLocalNotificationsPlugin.showWeeklyAtDayAndTime(
      0,
      'Movie App',
      'It\'s time for Watching ${MovieDetails[0]['title'].toString()} Movie!',
      day,
      time,
  ...
  Day convertStringToDay(String dayOfWeekString) {
    switch (dayOfWeekString.toLowerCase()) {
      ...
      case 'sunday':
        return Day.sunday;
      default:
        throw Exception('Invalid day of the week string');
    }
  }
}
